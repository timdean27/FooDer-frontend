...
const CheckRestaurants = ({likedFoods , dislikedFoods, searchPrice, searchLocation , searchRadius}) => {
const [searchCriteria, setSearchCriteria] = useState({term: "", location: searchLocation, price: searchPrice, radius: searchRadius})
...
const loadedTerm = () =>{     
    if(searchCriteria.term ===""){
          setSearchCriteria({term: FoodSearchForOBJ.name, location: searchLocation, price: searchPrice, radius: searchRadius})
          console.log("searchCriteria",searchCriteria )    
    }
        return(  
          <div>
            <h1>Running Search for Restaurants that got '{FoodSearchForOBJ.name}'</h1>
            <h1>Showing Restaurants in price of {searchPrice}</h1>
            <h1>Showing Restaurants around {searchLocation}</h1>
            <h1>Showing Restaurants around {searchRadius}</h1>
          </div>
          )
        } 
